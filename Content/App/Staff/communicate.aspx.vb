...
Partial Class Admin_studentprofile
            ...
            Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)                con.Open()
                ...
                Else
                    ...
                    If Request.QueryString("id") <> Nothing Then
                        Dim cmd1a As New MySql.Data.MySqlClient.MySqlCommand("SELECT student from communicate where id = '" & Request.QueryString("id") & "'", con)
    ...
    Sub load_chats()
        Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)            con.Open()
            ...
            Dim cmd1 As New MySql.Data.MySqlClient.MySqlCommand("SELECT message, time, sender, id, receiver from communicate where student = '" & Session("studentadd") & "' and time >= '" & Convert.ToDateTime(DateTime.ParseExact(opendate, "dd/MM/yyyy", Nothing)).ToString("yyyy-MM-dd HH:mm:ss") & "' and time <= '" & Convert.ToDateTime(DateTime.ParseExact(closingdate, "dd/MM/yyyy", Nothing)).ToString("yyyy-MM-dd HH:mm:ss") & "' order by time", con)
            Dim student0 As MySql.Data.MySqlClient.MySqlDataReader = cmd1.ExecuteReader
            Dim chats As String = ""
            Dim mymsg As String = ""
            Dim emojis As New ArrayList
            ...
            Do While student0.Read
                ...
                For Each item In emojis
                    thismsg = thismsg.Replace("sssjjjkkklll" & counter.ToString & ";", item)
                    counter = counter + 1
                Next
                counter = Nothing
                ...
                If student0("receiver") <> par.getparent(Session("studentadd")) Then
                    chats = chats + "<div class='row'>" & _
                                        "<div class='author-chat' id='" & counter & "fs" & "' style='word-wrap:break-word;'>" & _
                                            "<h3>Parent <span class='chat-date'>" & timelag & " " & IIf(dob.Hour > 12, IIf(dob.Hour - 12 > 9, dob.Hour - 12, "0" & dob.Hour - 12), IIf(dob.Hour > 9, dob.Hour, "0" & dob.Hour)) & ":" & IIf(dob.Minute < 10, "0" & dob.Minute, dob.Minute) & IIf(dob.Hour > 12, " pm", " am") & "</span></h3>" & _
                                            "<p>" & thismsg & "</p>" & _
                                        "</div>" & _
                                    "</div>"
                ...
                Else
                    chats = chats + "<div class='row'>" & _
                                       "<div class='client-chat' id='" & counter & "fss" & "' style='word-wrap:break-word;'>" & _
                                           "<h3>You <span class='chat-date'>" & timelag & " " & IIf(dob.Hour > 12, IIf(dob.Hour - 12 > 9, dob.Hour - 12, "0" & dob.Hour - 12), IIf(dob.Hour > 9, dob.Hour, "0" & dob.Hour)) & ":" & IIf(dob.Minute < 10, "0" & dob.Minute, dob.Minute) & IIf(dob.Hour > 12, " pm", " am") & "</span></h3>" & _
                                           "<p>" & thismsg & "</p>" & _
                                       "</div>" & _
                                   "</div>"
        ...
        Try
            Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)                con.Open()
                ...
                If Not Hidden1.Value.ToString = "" Then
                    ...
                    Dim cmd1 As New MySql.Data.MySqlClient.MySqlCommand("insert into communicate (student, message, sender, receiver, time) values (?,?,?,?,?)", con)
                    cmd1.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("sender", Session("Studentadd")))
                    cmd1.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("receiver", Hidden1.Value.ToString))
                    cmd1.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("subject", Session("cla")))
                    cmd1.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("message", par.getparent(Session("studentadd"))))
                    cmd1.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("date", Convert.ToDateTime(Now).ToString("yyyy-MM-dd HH:mm:ss")))
                    cmd1.ExecuteNonQuery()
                    ...
                    Dim id As Integer = student0a(0)
                    student0a.Close()
                    logify.Notifications(ss, par.getparent(Session("studentadd")), Session("staffid"), textinfo.ToTitleCase(firstname(1).ToString.ToLower) & " Class Teacher", "~/content/parent/communicate.aspx?" & id, "Message")
