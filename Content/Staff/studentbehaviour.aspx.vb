        ...
        Try
            Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)                con.Open()
                For Each row As GridViewRow In Gridview2.Rows
                    ...
                    Dim i As TextBox = row.FindControl("TextBox1")
                    Dim trait As String = i.Text
                   
                    Dim db As New DB_Interface
                    Dim rid As String = db.Select_single("Select termtraits.ID from termtraits inner join traits on traits.id = termtraits.trait where termtraits.student = '" & Session("studentadd") & "' and termtraits.session = '" & Session("sessionid") & "' and traits.trait = '" & id & "'")
                    db.Non_Query("Update termtraits set value = '" & trait & "' where id = '" & rid & "'")
                ...
                cmd.ExecuteNonQuery()
                con.Close()            End Using            Show_Alert(True, "Traits Updated")
        Catch ex As Exception
            Show_Alert(False, logify.error_log(sender.ToString, Replace(ex.ToString, "'", "")))
        End Try
    End Sub
