...
Partial Class Default2
        ...
        Try
            ...
            Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)                con.open()
                If Not IsPostBack Then
                    Dim cmdLoadx As New MySql.Data.MySqlClient.MySqlCommand("SELECT class.class from ttname inner join class on ttname.class = class.id where ttname.id = '" & Request.QueryString("timetable") & "'", con)
                    Dim studentx As MySql.Data.MySqlClient.MySqlDataReader = cmdLoadx.ExecuteReader
                    cboDay.Items.Clear()
                    Do While studentx.Read
                        cboDay.Items.Add(studentx.Item(0).ToString)
                    Loop
                    studentx.Close()
                    Dim cmdLoad As New MySql.Data.MySqlClient.MySqlCommand("SELECT class.class from ttname inner join (class inner join classteacher on classteacher.class = class.id) on ttname.school = class.superior where ttname.id = '" & Request.QueryString("timetable") & "' and classteacher.teacher = '" & Session("staffid") & "'", con)
                    Dim student As MySql.Data.MySqlClient.MySqlDataReader = cmdLoad.ExecuteReader
                    Do While student.Read
                        cboDay.Items.Add(student.Item(0).ToString)
                    Loop
                    student.Close()
    ...
    Protected Sub cboDay_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboDay.SelectedIndexChanged
        Try
            Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)                con.Open()
                ...
                Dim classes As New MySql.Data.MySqlClient.MySqlCommand("Select timetable from class where class = '" & cboDay.Text & "'", con)
                Dim schclass As MySql.Data.MySqlClient.MySqlDataReader = classes.ExecuteReader

                schclass.Read()
                Dim x As Integer = schclass(0)
                schclass.Close()
                If x = 0 Then
                    load_time_table(Request.QueryString("timetable"))
                Else
    ...
    Protected Sub LinkButton1_Click(sender As Object, e As EventArgs) Handles LinkButton1.Click
        Response.Redirect("~/content/staff/classdetails.aspx")
    End Sub
End Class
