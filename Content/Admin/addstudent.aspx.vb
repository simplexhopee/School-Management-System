...
Partial Class Admin_adminpage
    ...
    End Sub
    Private Sub Update_Attendance(cla As Integer)
        Dim db As New DB_Interface
        Dim weeks As New ArrayList
        Dim dates As New ArrayList
        Dim a As Array = db.Select_Query("select date, week from attendance where class = '" & cla & "' and term = '" & Session("sessionid") & "'")
        For o = 0 To a.GetLength(1) - 2
            If Not dates.Contains(a(0, o)) Then
                dates.Add(a(0, o))
                weeks.Add(a(1, o))
            End If
        ...
        Dim jj As Integer = 0
        Dim f As Integer
        For f = 0 To dates.Count - 1
            db.Non_Query("Insert into attendance (week, student, term, date, morning, afternoon, punctual, class) values ('" & weeks(f) & "','" & txtid.text & "','" & Session("SessionId") & "','" & dates(f) & "','" & -Val(False) & "','" & -Val(False) & "', '" & -Val(False) & "', '" & cla & "')")
            jj += 1
        Next
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)
                    con.open()
                    Dim cmdLoad03 As New MySql.Data.MySqlClient.MySqlCommand("SELECT class from class Order by Id", con)
                    ...
                    Do While student03.Read
                        cboClass.Items.Add(student03.Item(0).ToString)
                    Loop
                    student03.Close()
        ...
        Try
            Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)
                ...
                If txtExistPPhone.Text <> "" Then
                    Dim comfirms As New MySql.Data.MySqlClient.MySqlCommand("Select * from parentprofile where phone = ? or parentid = '" & txtExistPPhone.Text & "'", con)
                    ...
                    If Not rcomfirms.HasRows Then
                        Show_Alert(False, "Parent does not exist. Check the ID/Phone number or add new parent.")
                        e.Cancel = True
                        Exit Sub
                    Else
                        ...
                        If hasguardian = False Then
                            ...
                            logify.Notifications("You have been made the parent of " & txtSurname.Text, txtPID.Text, Session("staffid"), "Admin", "~/content/parent/studentprofile.aspx", "")
                        Else
            ...
            Response.Redirect("~/content/admin/studentprofile.aspx")
    ...
    Protected Sub Wizard1_NextButtonClick(sender As Object, e As WizardNavigationEventArgs)
        Try
                ...
                Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)
                    ...
                    If Session("studentadd") <> Nothing Then
                        If (rcomfirm.HasRows = True And Session("studentadd") = txtID.Text) Or (rcomfirm.HasRows = False And Session("studentadd") <> txtID.Text) Then
                            ...
                            logify.Notifications(msg, txtID.Text, Session("staffid"), "Admin", "~/content/student/studentprofile.aspx", "")
                            logify.Notifications("The profile of " & par.getstuname(txtID.Text) & " was updated by Admin", par.getparent(txtID.Text), Session("staffid"), "Admin", "~/content/parent/studentprofile.aspx?" & txtID.Text, "")
                ...
                Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)
                    ...
                    If CheckBox1.Checked = True Then
                        ...
                        Else
                            ...
                            logify.Notifications("You are now a boarder in " & cboHostel.Text, txtID.Text, Session("staffid"), "Admin", "~/content/student/studentprofile.aspx", "")
                        End If
                    End If
                    If CheckBox2.Checked = True Then
                        ...
                        Else
                            ...
                            logify.Notifications("You are now on transport. Route = " & cboTransport.Text, txtID.Text, Session("staffid"), "Admin", "~/content/student/studentprofile.aspx", "")
                        End If
                    End If
            ...
            ElseIf Wizard1.ActiveStep Is clas Then
                If cboClass.Text = "Select Class" Then
                    Show_Alert(False, "Please select a class")
                    e.Cancel = True
                Else
                    Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)
                        con.open()
                    ...
                    add_traits()
                End If
                logify.log(Session("staffid"), txtSurname.Text & " was admitted into " & cboClass.Text)
                logify.Notifications("You are now a student of " & cboClass.Text, txtID.Text, Session("staffid"), "Admin", "~/content/student/classdetails.aspx", "")
                        con.Close()
                    End Using
End If
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Try
            Using con As New MySql.Data.MySqlClient.MySqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("PortalDBConnectionString").ConnectionString)
                con.open()
                Dim comfirm As New MySql.Data.MySqlClient.MySqlCommand("Select * from parentprofile where phone = ? or parentid = '" & txtExistPPhone.Text & "'", con)
                comfirm.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("ID", txtExistPPhone.Text))
                Dim rcomfirm As MySql.Data.MySqlClient.MySqlDataReader = comfirm.ExecuteReader
                rcomfirm.Read()
                If Not rcomfirm.HasRows Then
                    Show_Alert(False, "Parent does not exist. Check the ID/Phone number")
                    Exit Sub
                End If
