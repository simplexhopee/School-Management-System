...
Partial Class Admin_addclass
       ...
       If check.Check_Admin(Session("roles"), Session("usertype")) = False Then Response.Redirect("~/default.aspx")
        Try
            If Not IsPostBack Then
                ...
                cboCopyClass.Items.Clear()
                cboCopyClass.Items.Add("SELECT CLASS")
                Do While reader4a.Read
                    ...
                    cboCopyClass.Items.Add(reader4a.Item(1))
                Loop
                reader4a.Close()
    ...
    Protected Sub Wizard1_FinishButtonClick(sender As Object, e As WizardNavigationEventArgs) Handles Wizard1.FinishButtonClick
        Try
            ...
            Response.Redirect("~/content/admin/viewtimetable.aspx?timetable=" & Session("timetable") & "&class=0&school=" & school)
            con.Close()
    ...
    Protected Sub Wizard1_NextButtonClick1(sender As Object, e As WizardNavigationEventArgs) Handles Wizard1.NextButtonClick
        Try
            If Wizard1.StartNextButtonText = "FINISH" Then Response.Redirect("~/content/admin/managetimetable.aspx")
    ...
    Protected Sub radType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles radType.SelectedIndexChanged
        Try
            If radType.SelectedValue = "Automated" Then
    ...
    Protected Sub BtnManualAdd_Click(sender As Object, e As EventArgs) Handles BtnManualAdd.Click
        Try
            ...
            If DropDownList1.Text = "SELECT CLASS" Or txtID.Text = "" Or cboDay3.Text = "Select Day" Or txtStartManual.Text = "" Or txtEndManual.Text = "" Then
                ...
                Exit Sub
            Else
                Dim checkstart As Array = Split(txtStartManual.Text, ":")
                Dim endstart As Array = Split(txtEndManual.Text, ":")
                If checkstart.Length <> 2 Or endstart.Length <> 2 Then
                    Show_Alert(False, "Please enter a valid time. Format = HH:MM")
                    Exit Sub
                End If
               

                If Val(checkstart(0)) > Val(endstart(0)) Then
                    Show_Alert(False, "Period not valid")
                    Exit Sub
                ElseIf Val(checkstart(0)) = Val(endstart(0)) And Val(checkstart(1)) > Val(endstart(1)) Then
                    Show_Alert(False, "Period not valid")
                    Exit Sub
                ElseIf Val(checkstart(0)) = Val(endstart(0)) And Val(checkstart(1)) = Val(endstart(1)) Then
                    Show_Alert(False, "Period not valid")
                ...
                Dim schooltt As Integer
                If readerf0s.Read() Then schooltt = readerf0s(0)
            ...
            If ChkManualTutorial.Checked = True Then
                ...
                Else
                    ...
                    Dim teacher As String = reader0(2).ToString
                    reader0.Close()
                    Dim cmdLoad10 As New MySql.Data.MySqlClient.MySqlCommand("SELECT tperiods.activity, tperiods.id  from tperiods where timetable = '" & Session("timetable") & "' and day = '" & cboDay3.Text & "'order by tperiods.timestart", con)
                    ...
                    Do While student10.Read
                        If student10(0).ToString = "Tutorial" Then
                            tperiod = tperiod + 1
                        End If
                        If period = student10(1) Then
                            If student10(0).ToString = "Tutorial" Then tutorial = True
                            Exit Do
                        End If
                    Loop
                    student10.Close()
            ...
            Else
                    Dim cmdLoad0 As New MySql.Data.MySqlClient.MySqlCommand("SELECT id from class where class = '" & DropDownList1.Text & "'", con)
                    Dim reader0 As MySql.Data.MySqlClient.MySqlDataReader = cmdLoad0.ExecuteReader
                    reader0.Read()
                    ...
                    Dim period As Integer = 0
                    If reader3s.Read Then
                        Show_Alert(False, "Period exists")
                        Exit Sub
                    Else
                        reader3s.Close()
                        Dim cmd3z As New MySql.Data.MySqlClient.MySqlCommand("INSERT Into tperiods (timetable, day, timestart, timeend, activity) Values ('" & Session("timetable") & "', '" & cboDay3.Text & "', '" & txtStartManual.Text & "', '" & txtEndManual.Text & "', '" & txtManualTutorial.Text & "')", con)
            ...
            End If
            cboManualSubjects.Text = "SELECT SUBJECT"
            txtStartManual.Text = ""
            txtEndManual.Text = ""
    ...
    Private Sub load_time_table()
        ...
        For Each Day As String In days
            ...
            Do While student10.Read
                Dim a As Array = Split(student10(0).ToString, ":")
                Dim b As Array = Split(student10(1).ToString, ":")
                If firstday = True Then
                    If student10(2).ToString = "Tutorial" Then
                        ds.Columns.Add(a(0) & ":" & a(1) & " - " & b(0) & ":" & b(1))
                        ds.Rows(x).Item(j) = a(0) & ":" & a(1) & " - " & b(0) & ":" & b(1)
    ...
    Protected Sub btnManualremove_Click(sender As Object, e As EventArgs) Handles btnManualremove.Click
        Try
            If cboDay3.Text = "Select Day" Then
                Show_Alert(False, "Select a day")
                Exit Sub
            End If
            con.Open()
            Dim cmdLoad10 As New MySql.Data.MySqlClient.MySqlCommand("SELECT tperiods.timestart, tperiods.timeend, tperiods.activity  from tperiods where timetable = '" & Session("timetable") & "' and day = '" & cboDay3.Text & "'order by tperiods.timestart", con)
            Dim student10 As MySql.Data.MySqlClient.MySqlDataReader = cmdLoad10.ExecuteReader
            Dim j As Integer = 1
            Dim nontutorial As New ArrayList
            cboManualRemove.Items.Clear()
            cboManualRemove.Items.Add("Select Period")
            Do While student10.Read
                cboManualRemove.Items.Add(student10(0).ToString & " - " & student10(1).ToString)
            Loop
            student10.Close()
            pnlManualRemove.Visible = True
            con.Close()
        Catch ex As Exception
            Show_Alert(False, logify.error_log(sender.ToString, Replace(ex.ToString, "'", "")))
        End Try
    ...
    Protected Sub btnRemovePeriod_Click(sender As Object, e As EventArgs) Handles btnRemovePeriod.Click
        Try
            con.Open()

            If cboManualRemove.Text = "Select Period" Then
                Show_Alert(False, "Select period to remove")
                Exit Sub
            End If
           
            Dim a As Array = Split(cboManualRemove.Text, " - ")
            
            Dim period As Integer
            Dim cmd3w As New MySql.Data.MySqlClient.MySqlCommand("select id from tperiods where timetable = '" & Session("timetable") & "' and day = '" & cboDay3.Text & "' and timestart = '" & a(0) & "' and timeend = '" & a(1) & "'", con)
            Dim reader3w As MySql.Data.MySqlClient.MySqlDataReader = cmd3w.ExecuteReader
            reader3w.Read()
            period = reader3w(0)
            reader3w.Close()
            ...
            If tutorial = True Then
                Dim cmdInsert35 As New MySql.Data.MySqlClient.MySqlCommand("Delete from timetable where tname = '" & Session("timetable") & "' and day = '" & cboDay3.Text & "' and period = '" & tperiod & "'", con)
                cmdInsert35.ExecuteNonQuery()
            End If
            Dim cmd3z As New MySql.Data.MySqlClient.MySqlCommand("Delete from tperiods where id = '" & period & "'", con)
            cmd3z.ExecuteNonQuery()

            Dim cmd3zx As New MySql.Data.MySqlClient.MySqlCommand("Delete from classperiods where period = '" & period & "'", con)
            cmd3zx.ExecuteNonQuery()

            pnlManualRemove.Visible = False
            con.Close()
            load_time_table()
        Catch ex As Exception
            Show_Alert(False, logify.error_log(sender.ToString, Replace(ex.ToString, "'", "")))
    ...
    Protected Sub chkCopy_CheckedChanged(sender As Object, e As EventArgs) Handles chkCopy.CheckedChanged
        If chkCopy.Checked = True Then
            cboCopyClass.Enabled = True
        Else
            cboCopyClass.Enabled = False
        End If
    End Sub

    Protected Sub cboCopyClass_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboCopyClass.SelectedIndexChanged
        Try
            con.Open()
            If Session("timetable") = Nothing Then
                Dim cmdLoad0 As New MySql.Data.MySqlClient.MySqlCommand("SELECT id from class where class = '" & DropDownList1.Text & "'", con)
                Dim reader0 As MySql.Data.MySqlClient.MySqlDataReader = cmdLoad0.ExecuteReader
                reader0.Read()
                Dim clas As Integer = reader0(0)
                ...
                cmdInsert37.ExecuteNonQuery()
            End If
            Dim cmdLoad10 As New MySql.Data.MySqlClient.MySqlCommand("SELECT timetable, id from class where class = '" & cboCopyClass.Text & "'", con)
            Dim student10 As MySql.Data.MySqlClient.MySqlDataReader = cmdLoad10.ExecuteReader

            student10.Read()
            Dim timetable As Integer = student10(0)
            Dim claid As Integer = student10(1)
            student10.Close()


            If timetable = 0 Then
                Show_Alert(False, "No time table exists for the selected class")
                chkCopy.Checked = False
                cboClasses.Text = "SELECT CLASS"
                cboCopyClass.Enabled = False
                Exit Sub
            Else
                Dim cmdLoad10A As New MySql.Data.MySqlClient.MySqlCommand("SELECT subject from classsubjects where class = '" & claid & "'", con)
                Dim student10A As MySql.Data.MySqlClient.MySqlDataReader = cmdLoad10A.ExecuteReader
                Dim copyclass As New ArrayList
                Do While student10A.Read()
                    copyclass.Add(student10A(0))
                Loop
                student10A.Close()
                ...
                Dim cmdLoad10Ax As New MySql.Data.MySqlClient.MySqlCommand("SELECT subject from classsubjects where class = '" & clas & "'", con)
                Dim student10Ax As MySql.Data.MySqlClient.MySqlDataReader = cmdLoad10Ax.ExecuteReader
                Dim copyclass2 As New ArrayList
                Dim allsame As Boolean = False

                Do While student10Ax.Read()
                    copyclass2.Add(student10Ax(0))
                Loop
                student10Ax.Close()
                For Each item As Integer In copyclass
                    Dim somesame As Boolean = False
                    For Each subitem As Integer In copyclass2
                ...
                If allsame = False Or copyclass.Count <> copyclass2.Count Then
                    Show_Alert(False, "Subjects offerred both classes are not the same")
                    Exit Sub
                Else
                    Dim cmdLoaddfs As New MySql.Data.MySqlClient.MySqlCommand("delete from timetable where tname = '" & Session("timetable") & "'", con)
                    cmdLoaddfs.ExecuteNonQuery()

                    Dim cmdLoaddfss As New MySql.Data.MySqlClient.MySqlCommand("delete from tperiods where timetable = '" & Session("timetable") & "'", con)
                    cmdLoaddfss.ExecuteNonQuery()
                    Dim cmdLoad10Axa As New MySql.Data.MySqlClient.MySqlCommand("SELECT period, subject, day from timetable where tname = '" & timetable & "'", con)
                    Dim student10Axa As MySql.Data.MySqlClient.MySqlDataReader = cmdLoad10Axa.ExecuteReader
                    Dim period As New ArrayList
                    Dim subject As New ArrayList
                    Dim day As New ArrayList


                    Do While student10Axa.Read
                        period.Add(student10Axa(0))
                        subject.Add(student10Axa(1))
                        day.Add(student10Axa(2))
                    Loop
                    student10Axa.Close()
                    Dim ct As Integer = 0
                    For Each item As String In subject
                        Dim cmdLoad1d As New MySql.Data.MySqlClient.MySqlCommand("SELECT teacher from classsubjects where subject = '" & item & "' and class = '" & clas & "'", con)
                        Dim student1d As MySql.Data.MySqlClient.MySqlDataReader = cmdLoad1d.ExecuteReader
                        student1d.Read()
                        Dim teacher As String = student1d(0).ToString
                        student1d.Close()
                        Dim cmdInsert35 As New MySql.Data.MySqlClient.MySqlCommand("INSERT Into timetable (tname, Class, Period, Subject, Day, Teacher) Values(?,?,?,?,?,?)", con)
                        cmdInsert35.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("ss", Session("timetable")))
                        cmdInsert35.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("class", clas))
                        cmdInsert35.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("Period", period(ct)))
                        cmdInsert35.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("subject", item))
                        cmdInsert35.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("Doubles", day(ct)))
                        cmdInsert35.Parameters.Add(New MySql.Data.MySqlClient.MySqlParameter("Day", teacher))
                        cmdInsert35.ExecuteNonQuery()
                        ct = ct + 1
                    Next

                    Dim cmdLoadd As New MySql.Data.MySqlClient.MySqlCommand("SELECT day, period, timestart, timeend, activity from tperiods where timetable = '" & timetable & "'", con)
                    Dim studentd As MySql.Data.MySqlClient.MySqlDataReader = cmdLoadd.ExecuteReader
                    Dim period2 As New ArrayList
                    Dim day2 As New ArrayList
                    Dim timestart2 As New ArrayList
                    Dim timesend2 As New ArrayList
                    Dim activity2 As New ArrayList
                    Do While studentd.Read

                        day2.Add(studentd(0))
                        timestart2.Add(studentd(2).ToString)
                        timesend2.Add(studentd(3).ToString)
                        activity2.Add(studentd(4))
                    Loop
                    studentd.Close()
                    Dim ct2 As Integer
                    For Each item As String In day2
                        Dim cmdLoaddf As New MySql.Data.MySqlClient.MySqlCommand("Insert into tperiods (day,  timestart, timeend, activity, timetable) values ('" & day2(ct2) & "', '" & timestart2(ct2) & "', '" & timesend2(ct2) & "', '" & activity2(ct2) & "', '" & Session("timetable") & "')", con)
                        cmdLoaddf.ExecuteNonQuery()
                        ct2 = ct2 + 1
                    Next

                    Show_Alert(True, "Time table copied")
                End If
